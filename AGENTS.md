...
- **No Complicated Tool Calls:** Stop prompting the user for permission on complicated or convoluted shell commands. Keep tool operations simple and direct. Do not run multi-step pipeline shell commands, complex PowerShell invocations, or commands that trigger incessant user prompts when simple standard tools or standard targets (`make test`, standard file tools) suffice.
