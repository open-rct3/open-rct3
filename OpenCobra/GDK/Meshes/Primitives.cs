...
public static class Primitives {
  /// <summary>
  /// Create a flat quad on the XZ plane (Y-up).
  /// </summary>
  ...
  public static Mesh Plane(string? name = null, Vector4 color = default) => new([
    new Vertex { Position = new Vector3(-1, 0, -1), TexCoord = new Vector2(0, 0), Color = color, Normal = Vector3.UnitY },
    new Vertex { Position = new Vector3( 1, 0, -1), TexCoord = new Vector2(1, 0), Color = color, Normal = Vector3.UnitY },
    new Vertex { Position = new Vector3( 1, 0,  1), TexCoord = new Vector2(1, 1), Color = color, Normal = Vector3.UnitY },
    new Vertex { Position = new Vector3(-1, 0,  1), TexCoord = new Vector2(0, 1), Color = color, Normal = Vector3.UnitY }
  ], [0, 3, 2, 0, 2, 1]) { Name = name };

  /// <summary>
  /// Create an axis-aligned unit cube (extents -1..1 on each axis, Y-up), one flat-shaded quad per face.
  /// </summary>
  /// <param name="name">Name assigned to <see cref="Mesh.Name"/></param>
  /// <param name="color">
  /// Vertex color assigned to <see cref="Vertex.Color"/>. Defaults to <see cref="Colors.Transparent"/>.
