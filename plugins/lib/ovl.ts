...
/**
 * @internal
 * Returns a `Memory` instance for the given offset, using `@extism/as-pdk`'s `length`/`load_u8`
 * built-ins to obtain the memory's length.
 */
...
/**
 * Provides access to OVL data via host functions exposed by Dumper's ViewerPlugin.cs.
 */
export class Ovl {
  /**
