...
Plugins are discovered at startup from bundled (`bin/plugins/`) and user data directories. The first matching plugin for each file type is the default viewer.
...
Plugins are planned in priority order based on implementation difficulty (from the [OVL Decoding plans](.opencode/plans/OVL%20Decoding/)):
...
export function name(): string { return "Example Viewer"; }
export function version(): string { return "0.1.0"; }
export function file_types(): string { return '["xxx"]'; }
