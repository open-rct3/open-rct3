...
const wasmUrl = new URL("../../bin/plugins/txt-viewer.wasm", import.meta.url);

Deno.test("txt-viewer: name()", async () => {
  const plugin = await createPlugin(wasmUrl, { functions });
  const out = await plugin.call("name");
  assert(out !== null, "Expected a result!");
  assertEquals(out!.text(), "Text Viewer");
  await plugin.close();
});

Deno.test("txt-viewer: version()", async () => {
  const plugin = await createPlugin(wasmUrl, { functions });
...
Deno.test("txt-viewer: file_types()", async () => {
  const plugin = await createPlugin(wasmUrl, { functions });
  const out = await plugin.call("file_types");
  assert(out !== null, "Expected a result!");
  assertEquals(JSON.parse(out!.text()), ["txt"]);
  await plugin.close();
});
